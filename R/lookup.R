...
#'  (That is, higher (conceptually) than `lookup`; aka larger than `lookup`!).
#'  If not supplied, this uses the default `NULL`, and just data for `lookup`
#'  will be returned. In this case, `within_names` and `within_codes` (if
#'  supplied) will instead be taken to refer to, and used to filter at, the
#'  `lookup` level.
...
#'  specified! Otherwise `{boundr}` will apply them to `lookup` instead.
#'  See examples.
...
#'  to `lookup`, if you need it. Defaults to `NULL`, which then aims to return
#'  data for the most recent year available. Provide a full 4 digit number.
