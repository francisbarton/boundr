test_that("arrange_service_names_by_res_codes works", {
  # c(
  ...
  t3 <- purrr::map(t1[["service_name"]], \(x) purrr::map_lgl(res_codes(), \(cd) grepl(cd, x)))
  expect_length(t3, nrow(t1))
  t4 <- purrr::map_int(t1[["service_name"]], \(x) min(if (length(v <- which(purrr::map_lgl(res_codes(), \(cd) grepl(cd, x)))) == 0) nrow(t1) else v))
  ...
  t7 <- arrange_service_names_by_res_codes(t1)
